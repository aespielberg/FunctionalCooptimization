...
p = RigidBodyManipulator('QuadcopterParams.urdf',options);
...
N = 20; %TODO: bump this back up
prog = DirtranTrajectoryOptimizationTest(p,N,[0 tf0]);
prog = prog.setRegularizerWeight(0.0);
options.penalty = 2;
prog.actuation_weight = 1.0;
prog = prog.setOptimizationType(options);

thresh = 0.000;
prog = prog.addStateConstraint(ConstantConstraint(x0),1);
prog = prog.addStateConstraint(ConstantConstraint(x1),floor(N/3));
prog = prog.addStateConstraint(ConstantConstraint(x2),floor(2 * N / 3));
