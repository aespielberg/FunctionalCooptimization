classdef AcrobotPlantTest < RigidBodyManipulator
    ...
    methods
        function obj = AcrobotPlantTest
            obj = obj@RigidBodyManipulator('AcrobotWParamsGeometric.urdf');
...
%             prog = prog.addFinalCost(@finalCost);
        ...
        function [xtraj,z,F,info]=swingUpTrajectoryKinematic(obj)
            x0 = zeros(4,1);
            xf = double([pi;0;0;0]);
            tf0 = 4;
            
            N = 21;
            prog = KinematicDirtranTest(obj,N,[0 6]);
            prog = prog.addStateConstraint(ConstantConstraint(x0),1);
            constraint = constructPositionDiffConstraint( [0; -inf; 0; -inf; -inf; -inf]...
                ,[0; inf; 0; inf; inf; inf],obj, 'lower_link', [0; 0; 0.0],...
                [0; 0; 10; 0; 0; 0]);
            ...
            prog = prog.addRunningCost(@running_cost_function);
            ...
            function [g, dg] = running_cost_function(dt, x, u, params)
                g = 0;
                dg = [0 zeros(1, length(x)), zeros(1, length(u)), zeros(1, length(params))];
            end
