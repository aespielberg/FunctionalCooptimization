...
BASE_LINK_EXTENTS_VAL = np.array([1.0, 1.0, 1.0])
...
for i in range(NUM_LEGS):
    LEG_MASS.append("$LEG_MASS" + str(i))
    LEG_MASS_VAL.append(1.0)
BASE_LINK_EXTENTS = ["$BASE_LINK_EXTENTS_X", "$BASE_LINK_EXTENTS_Y", "$BASE_LINK_EXTENTS_Z"]

LEG_LINK_EXTENTS = []
for i in range(NUM_LEGS):
    LEG_LINK_EXTENTS.append(["$LEG_LINK_EXTENTS_X" + str(i), "$LEG_LINK_EXTENTS_Y" + str(i), "$LEG_LINK_EXTENTS_Z" + str(i)])
LEG_LINK_EXTENTS_VAL = np.array([[0.3, 0.3, 2.0], [0.3, 0.3, 1.0]])
...
leg_mass_param = []
for i in NUM_LEGS:
    leg_mass_param.append([LEG_MASS[i], LEG_MASS_VAL[i]])
...
leg_link_extents_param_x = []
leg_link_extents_param_y = []
leg_link_extents_param_z = []

for i in range(NUM_LEGS):
    leg_link_extents_param_x.append([LEG_LINK_EXTENTS[i][0], LEG_LINK_EXTENTS_VAL[i][0]])
    leg_link_extents_param_y.append([LEG_LINK_EXTENTS[i][1], LEG_LINK_EXTENTS_VAL[i][1]])
    leg_link_extents_param_z.append([LEG_LINK_EXTENTS[i][2], LEG_LINK_EXTENTS_VAL[i][2]])
...
def add_parameters(robot):
    ...
    if isinstance(leg_mass_param[0][0], str):
        param = ET.SubElement(robot, "parameter")
        param.attrib["name"] = leg_mass_param[0][0][1:] #fully constrained parameter
        param.attrib["value"] = str(leg_mass_param[0][1])
        param.attrib["lb"] = "0.1"
        param.attrib["ub"] = "5.0"
        leg_mass_param[0][0] = symbols(leg_mass_param[0][0])
